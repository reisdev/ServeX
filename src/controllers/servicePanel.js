...
export class ServicePanel
{
	...
	{
		return (response) => response.render('servicePanel/stopgap.pug', {
			title,
			message,
			links: [
				... links,
	...
	{
		return response.render('servicePanel/index.pug')
	}
	...
	static async pending ({ params, session }, response)
	{
		...
		return response.render('servicePanel/pending.pug', { moment, services, translatePricing })
	}
	...
	static async active ({ params, session }, response)
	{
		...
		return response.render('servicePanel/active.pug', { moment, services, translatePricing })
	}
	...
	static async history ({ params, session }, response)
	{
		...
		return response.render('servicePanel/history.pug', {
	...
	static async accept ({ params, session }, response)
	{
		...
		await sequelize.transaction(async (transaction) => {
			...
			return ServicePanel.stopgap(`Contrato ${accepted ? 'aceito' : 'recusado'}.`)(response)
		})
	}
	...
	static async complete ({ params, session }, response)
	{
		await sequelize.transaction(async (transaction) => {
			...
			return ServicePanel.stopgap('Serviço concluído.')(response)
		})
	}
}
