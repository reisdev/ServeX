...
@Router.Route('/hire', [
	Middlewares.restrictedPage({
		message: 'Efetue login para contratar um serviço.'
	})
])
export class Hire {
	...
	static die(error, message) {
		return (response) => response.render('error.pug', {
			error,
			message
		})
	}
	...
	static async index({
		...
		if(!session.user)
			return Hire.die('Nenhum usuário conectado','Efetue login para poder contratar')(response)
		
		const addresses = await Hire.findAddressByUserId(session.user.id)
		console.log(addresses)

		return Hire.render(service,addresses)(response)
	}
