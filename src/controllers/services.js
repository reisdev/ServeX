...
export class Service
{
	...
	static async contract({ params, body, session }, response)
	{
		...
		if (_.isEmpty(body))
			return response.render('hireService.pug', {
				service, mapPricingType
			})
		...
		if (! body.date || ! body.time || ! body.timefactor)
			return response.render('hireService.pug', {
				service, mapPricingType, error: [ 'Preencha todos os campos.' ]
			})
		...
		try {
			...
			return response.status(400).render('hireService.pug', {
				service, mapPricingType, error: ['Erro ao contratar serviço'],
			})
		}
	}
}
